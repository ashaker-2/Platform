 ...
 * @return APP_OK on success, APP_ERROR if any pin fails to initialize.
 */
...
sequenceDiagram
    ...
    loop For each pin_config in hal_gpio_initial_config
        ...
        alt MCAL_GPIO_Init returns MCAL_ERROR
            ...
            HAL_GPIO--xRTE: Return APP_ERROR (after attempting all pins)
        else MCAL_GPIO_Init returns MCAL_OK
            MCAL_GPIO-->>HAL_GPIO: Return MCAL_OK
        end
    end
    HAL_GPIO-->>RTE: Return APP_OK (if all successful)
```
...
- **Return Status:** All public API functions will return `APP_ERROR` on failure. `HAL_GPIO_Init` will return `APP_ERROR` if any pin fails to initialize.  
...
- **Test Cases:**  
  - `HAL_GPIO_Init`: Test with a valid `hal_gpio_initial_config` array. Verify that `MCAL_GPIO_Init` is called for each entry. Test scenarios where `MCAL_GPIO_Init` fails for one or more pins (verify `APP_ERROR` return and SystemMonitor fault reporting).  
