...
// #include "uart.h"          // MCAL layer for direct UART hardware access
...
Status_t HAL_ModbusRTU_Init(const HAL_ModbusRTU_Config_t *config_p) 
{
    ...
    // if (MCAL_UART_Init(MCAL_UART_INSTANCE_MODBUS, &mcal_uart_config, &mcal_uart_cbs) != E_OK) {
    ...
    // if (MCAL_UART_Receive_IT(MCAL_UART_INSTANCE_MODBUS, s_modbus_rtu_state.rx_buffer, 1) != E_OK) {
...
Status_t HAL_ModbusRTU_SendFrame(const uint8_t *frame_p, uint16_t length) {
    ...
    // if (MCAL_UART_Transmit_IT(MCAL_UART_INSTANCE_MODBUS, s_modbus_rtu_state.tx_buffer, length + 2) != E_OK) {
