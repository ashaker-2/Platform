...
#include "mcal_bluetooth.h"     // MCAL layer for direct Bluetooth hardware access
...
static void MCAL_BLE_StateChangeHandler(MCAL_BLE_State_t mcal_state);
...
HAL_Status_t HAL_BLE_Init(HAL_BLE_Role_t role) {
    ...
    if (MCAL_BLE_Init(mcal_role, &mcal_cbs) != APP_OK) {
...
HAL_Status_t HAL_BLE_StartAdvertising(const HAL_BLE_AdvParams_t *adv_params) {
    ...
    if (MCAL_BLE_StartAdvertising(&mcal_adv_params) != APP_OK) {
...
HAL_Status_t HAL_BLE_StopAdvertising(void) {
    ...
    if (MCAL_BLE_StopAdvertising() != APP_OK) {
...
HAL_Status_t HAL_BLE_StartScanning(const HAL_BLE_ScanParams_t *scan_params) {
    ...
    if (MCAL_BLE_StartScanning(&mcal_scan_params) != APP_OK) {
...
HAL_Status_t HAL_BLE_StopScanning(void) {
    ...
    if (MCAL_BLE_StopScanning() != APP_OK) {
...
HAL_Status_t HAL_BLE_Connect(const uint8_t *peer_address, const HAL_BLE_ConnParams_t *conn_params) {
    ...
    if (MCAL_BLE_Connect(peer_address, &mcal_conn_params) != APP_OK) {
...
HAL_Status_t HAL_BLE_Disconnect(void) {
    ...
    if (!s_ble_module_state.is_connected) {
        ...
        return HAL_OK; // Already disconnected, consider it success
    }

    if (MCAL_BLE_Disconnect() != APP_OK) {
...
HAL_Status_t HAL_BLE_SendData(const uint8_t *data_p, uint16_t length) {
    ...
    if (MCAL_BLE_SendData(data_p, length) != APP_OK) {
