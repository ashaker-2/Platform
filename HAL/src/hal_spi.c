...
#include "hal_spi.h"
#include "hal_spi_cfg.h"     // Include configuration header
// #include "spi_master.h" // ESP-IDF SPI driver
// #include "gpio.h"       // ESP-IDF GPIO driver
...
typedef struct
{
    HAL_SPI_Status_t spi_handle;      /**< Handle to the ESP-IDF SPI device. */
    ...
    HAL_SPI_Status_t transfer_done_sem; /**< Semaphore to signal completion of asynchronous transfers. */
 ...
 */
static void spi_transaction_callback(HAL_SPI_Mode_t *trans)
{
 ...
 */
static HAL_SPI_Mode_t hal_spi_mode_to_esp_mode(HAL_SPI_Mode_t mode)
{
    // switch (mode)
    // {
    // case HAL_SPI_MODE_0:
 ...
 */
static HAL_SPI_Channel_t hal_spi_channel_to_esp_host(HAL_SPI_Channel_t channel)
{
    // switch (channel)
    // {
