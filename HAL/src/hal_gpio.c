...
#include "mcal_gpio.h"      // MCAL layer for direct GPIO hardware access
...
static void HAL_GPIO_McalInterruptHandler(MCAL_GPIO_Pin_t mcal_pin);
static HAL_GPIO_Pin_t HAL_GPIO_GetLogicalPin(MCAL_GPIO_Pin_t mcal_pin);
...
HAL_Status_t HAL_GPIO_Init(void) {
    ...
    for (uint8_t i = 0; i < HAL_GPIO_PIN_COUNT; i++) {
        ...
        HAL_Status_t status = HAL_GPIO_Config(config->logical_id, config->default_direction,
        ...
        if (status != HAL_OK) {
            ...
            // For critical pins, this might lead to HAL_ERROR return
        }
    }
    ...
    for (uint8_t i = 0; i < HAL_GPIO_PIN_COUNT; i++) {
        ...
        if (config->is_interrupt_capable) {
            ...
            if (MCAL_GPIO_RegisterInterruptCallback(config->mcal_pin, HAL_GPIO_McalInterruptHandler) != APP_OK) {
...
HAL_Status_t HAL_GPIO_Config(HAL_GPIO_Pin_t pin, HAL_GPIO_Direction_t direction,
    ...
    if (MCAL_GPIO_Config(config->mcal_pin, mcal_dir, mcal_initial_state, mcal_pull) != APP_OK) {
...
HAL_Status_t HAL_GPIO_Write(HAL_GPIO_Pin_t pin, HAL_GPIO_State_t state) {
    ...
    if (MCAL_GPIO_Write(config->mcal_pin, mcal_state) != APP_OK) {
...
HAL_Status_t HAL_GPIO_Read(HAL_GPIO_Pin_t pin, HAL_GPIO_State_t *state_p) {
    ...
    if (MCAL_GPIO_Read(config->mcal_pin, &mcal_state) != APP_OK) {
...
HAL_Status_t HAL_GPIO_ConfigInterrupt(HAL_GPIO_Pin_t pin, HAL_GPIO_InterruptType_t interrupt_type,
    ...
    if (MCAL_GPIO_ConfigInterrupt(config->mcal_pin, mcal_int_type) != APP_OK) {
...
HAL_Status_t HAL_GPIO_EnableInterrupt(HAL_GPIO_Pin_t pin) {
    ...
    const HAL_GPIO_Config_t *config = &g_hal_gpio_configs[pin];
    if (MCAL_GPIO_EnableInterrupt(config->mcal_pin) != APP_OK) {
...
HAL_Status_t HAL_GPIO_DisableInterrupt(HAL_GPIO_Pin_t pin) {
    ...
    const HAL_GPIO_Config_t *config = &g_hal_gpio_configs[pin];
    if (MCAL_GPIO_DisableInterrupt(config->mcal_pin) != APP_OK) {
