...
#include "mcal_pwm.h"       // MCAL layer for direct PWM hardware access
...
HAL_Status_t HAL_PWM_Init(void) {
    ...
    for (uint8_t i = 0; i < HAL_PWM_CHANNEL_COUNT; i++) {
        ...
        HAL_Status_t gpio_status = HAL_GPIO_Config(config->gpio_pin, HAL_GPIO_DIR_OUTPUT,
        ...
        HAL_Status_t status = HAL_PWM_ConfigChannel(config->logical_id,
...
HAL_Status_t HAL_PWM_ConfigChannel(HAL_PWM_Channel_t channel,
    ...
    if (MCAL_PWM_Config(config->mcal_timer, config->mcal_channel,
                        frequency_hz, initial_duty_cycle_percent) != APP_OK) {
...
HAL_Status_t HAL_PWM_SetDutyCycle(HAL_PWM_Channel_t channel, uint8_t duty_cycle_percent) {
    ...
    if (MCAL_PWM_SetDutyCycle(config->mcal_timer, config->mcal_channel, duty_cycle_percent) != APP_OK) {
...
HAL_Status_t HAL_PWM_Start(HAL_PWM_Channel_t channel) {
    ...
    if (MCAL_PWM_Start(config->mcal_timer, config->mcal_channel) != APP_OK) {
...
HAL_Status_t HAL_PWM_Stop(HAL_PWM_Channel_t channel) {
    ...
    if (MCAL_PWM_Stop(config->mcal_timer, config->mcal_channel) != APP_OK) {
