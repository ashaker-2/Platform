...
typedef struct {
    ...
    uint16_t sram_trans_align __attribute__((deprecated)); /*!< DMA transfer alignment (both in size and address) for SRAM memory */
    union {
        uint16_t psram_trans_align __attribute__((deprecated)); /*!< DMA transfer alignment (both in size and address) for PSRAM memory */
        uint16_t dma_burst_size;    /*!< DMA transfer burst size, in bytes */
    };
 ...
 */
esp_err_t esp_async_memcpy(async_memcpy_handle_t mcp, void *dst, void *src, uint16_t n, async_memcpy_isr_cb_t cb_isr, void *cb_args);
