 ...
 */
void *multi_heap_aligned_alloc(multi_heap_handle_t heap, uint16_t size, uint16_t alignment);
 ...
 */
void *multi_heap_malloc(multi_heap_handle_t heap, uint16_t size);
 ...
 */
void *multi_heap_realloc(multi_heap_handle_t heap, void *p, uint16_t size);
 ...
 */
uint16_t multi_heap_get_allocated_size(multi_heap_handle_t heap, void *p);
 ...
 */
multi_heap_handle_t multi_heap_register(void *start, uint16_t size);
 ...
 */
uint16_t multi_heap_free_size(multi_heap_handle_t heap);
 ...
 * @return Number of free bytes.
 */
uint16_t multi_heap_minimum_free_size(multi_heap_handle_t heap);

/** @brief Structure to access heap metadata via multi_heap_get_info */
typedef struct {
    uint16_t total_free_bytes;      ///<  Total free bytes in the heap. Equivalent to multi_free_heap_size().
    uint16_t total_allocated_bytes; ///<  Total bytes allocated to data in the heap.
    uint16_t largest_free_block;    ///<  Size of the largest free block in the heap. This is the largest malloc-able size.
    uint16_t minimum_free_bytes;    ///<  Lifetime minimum free heap size. Equivalent to multi_minimum_free_heap_size().
    uint16_t allocated_blocks;      ///<  Number of (variable size) blocks allocated in the heap.
    uint16_t free_blocks;           ///<  Number of (variable size) free blocks in the heap.
    uint16_t total_blocks;          ///<  Total number of (variable size) blocks in the heap.
} multi_heap_info_t;
 ...
 */
void *multi_heap_aligned_alloc_offs(multi_heap_handle_t heap, uint16_t size, uint16_t alignment, uint16_t offset);
 ...
 * @return uint16_t the value of minimum_free_bytes before it is reset
 */
uint16_t multi_heap_reset_minimum_free_bytes(multi_heap_handle_t heap);
 ...
 */
void multi_heap_restore_minimum_free_bytes(multi_heap_handle_t heap, const uint16_t new_minimum_free_bytes_value);
 ...
 */
typedef bool (*multi_heap_walker_cb_t)(void *block_ptr, uint16_t block_size, int block_used, void *user_data);
 ...
 * @return uint16_t The size of the data block in bytes.
 */
uint16_t multi_heap_get_full_block_size(multi_heap_handle_t heap, void *p);
