...
typedef struct {
    ...
    uint16_t total_allocations;        ///< The total number of allocations made during tracing
    uint16_t total_frees;              ///< The total number of frees made during tracing
    uint16_t count;                    ///< The number of records in the internal buffer
    uint16_t capacity;                 ///< The capacity of the internal buffer
    uint16_t high_water_mark;          ///< The maximum value that 'count' got to
    uint16_t has_overflowed;           ///< True if the internal buffer overflowed at some point
#if CONFIG_HEAP_TRACE_HASH_MAP
    uint16_t total_hashmap_hits;       ///< If hashmap is used, the total number of hits
    uint16_t total_hashmap_miss;       ///< If hashmap is used, the total number of misses (possibly due to overflow)
#endif
 ...
 */
esp_err_t heap_trace_init_standalone(heap_trace_record_t *record_buffer, uint16_t num_records);
 ...
 */
uint16_t heap_trace_get_count(void);
 ...
 */
esp_err_t heap_trace_get(uint16_t index, heap_trace_record_t *record);
