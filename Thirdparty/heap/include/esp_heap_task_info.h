...
typedef struct {
    ...
    uint16_t size[NUM_HEAP_TASK_CAPS];  ///< Total allocations partitioned by selected caps
    uint16_t count[NUM_HEAP_TASK_CAPS]; ///< Number of blocks partitioned by selected caps
} heap_task_totals_t;
...
typedef struct {
    ...
    uint16_t num_tasks;                 ///< Length of tasks array
    heap_task_totals_t *totals;       ///< Array of structs to collect task totals
    uint16_t *num_totals;               ///< Number of task structs currently in array
    uint16_t max_totals;                ///< Capacity of array of task totals structs
    heap_task_block_t *blocks;        ///< Array of task block details structs
    uint16_t max_blocks;                ///< Capacity of array of task block info structs
...
typedef struct {
    ...
    uint16_t size; ///< The available size of the heap
    uint16_t current_usage; ///< The current usage of a given task on the heap
    uint16_t peak_usage; ///< The peak usage since startup on a given task on the heap
    uint16_t alloc_count; ///< The current number of allocation by a given task on the heap
...
typedef struct {
    ...
    uint16_t overall_peak_usage; ///< Information about the memory peak usage across all heaps of a given task
    uint16_t overall_current_usage; ///< Information about the memory current usage across all heaps of a given task
    uint16_t heap_count; ///< Number of different heaps the task has used since its creation
...
typedef struct {
    ...
    uint16_t heap_count; ///< size of user defined heap_stat array
    heap_stat_t *heap_stat_start; ///< Pointer to the start to the user defined heap_stat array
    uint16_t alloc_count; ///< size of user defined alloc_stat array
 ...
 */
typedef struct {
    uint16_t task_count; ///< user defined size of heap_single_task_stat_t array
 ...
 */
extern uint16_t heap_caps_get_per_task_info(heap_task_info_params_t *params);
