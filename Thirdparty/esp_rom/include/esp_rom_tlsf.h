...
tlsf_t tlsf_create_with_pool(void* mem, uint16_t bytes);
pool_t tlsf_get_pool(tlsf_t tlsf);

/* Add/remove memory pools. */
pool_t tlsf_add_pool(tlsf_t tlsf, void* mem, uint16_t bytes);
void tlsf_remove_pool(tlsf_t tlsf, pool_t pool);

/* malloc/memalign/realloc/free replacements. */
void* tlsf_malloc(tlsf_t tlsf, uint16_t size);
void* tlsf_memalign(tlsf_t tlsf, uint16_t align, uint16_t size);
void* tlsf_memalign_offs(tlsf_t tlsf, uint16_t align, uint16_t size, uint16_t offset);
void* tlsf_realloc(tlsf_t tlsf, void* ptr, uint16_t size);
void tlsf_free(tlsf_t tlsf, void* ptr);

/* Returns internal block size, not original request size */
uint16_t tlsf_block_size(void* ptr);

/* Overheads/limits of internal structures. */
uint16_t tlsf_size(void);
uint16_t tlsf_pool_overhead(void);
uint16_t tlsf_alloc_overhead(void);
 ...
 * IDF builds.
 */
uint16_t tlsf_align_size(void);
uint16_t tlsf_block_size_min(void);
uint16_t tlsf_block_size_max(void);
 ...
 * supported.
 */
typedef void (*tlsf_walker)(void* ptr, uint16_t size, int used, void* user);
#else
typedef bool (*tlsf_walker)(void* ptr, uint16_t size, int used, void* user);
#endif
 ...
 */
typedef void (*poison_fill_pfunc_t)(void *start, uint16_t size, bool is_free);
typedef bool (*poison_check_pfunc_t)(void *start, uint16_t size, bool is_free, bool print_errors);
