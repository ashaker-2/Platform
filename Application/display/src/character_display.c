...
{
    if (lcd_send_byte(unit_id, cmd, 0) != APP_OK)
    { // RS low for command
...
{
    if (lcd_send_byte(unit_id, data, RS_BIT) != APP_OK)
    { // RS high for data
...
uint8_t CHARACTER_DISPLAY_Init(void)
{
    ...
    for (uint32_t i = 0; i < CHARACTER_DISPLAY_NUM_CONFIGURATIONS; i++)
    {
        ...
        if (lcd_send_nibble(i, 0x30, 0) != APP_OK)
        {
            app_ret = APP_ERROR;
            continue;
        }
        ...
        if (lcd_send_nibble(i, 0x20, 0) != APP_OK)
        {
            app_ret = APP_ERROR;
            continue;
        }                               // Sets 4-bit mode
        ...
        if (lcd_send_cmd(i, LCD_FUNCTIONSET | state->display_function_flags) != APP_OK)
        {
            app_ret = APP_ERROR;
            continue;
        }
        ...
        if (lcd_send_cmd(i, LCD_DISPLAYCONTROL | state->display_control_flags) != APP_OK)
        {
            app_ret = APP_ERROR;
            continue;
        }
        ...
        // Clear Display
        if (lcd_send_cmd(i, LCD_CLEARDISPLAY) != APP_OK)
        {
            app_ret = APP_ERROR;
            continue;
        }
        ...
        if (lcd_send_cmd(i, LCD_ENTRYMODESET | LCD_ENTRYSHIFTINCREMENT | LCD_ENTRYLEFT) != APP_OK)
        {
            app_ret = APP_ERROR;
            continue;
        }
        ...
        // Return Home
        if (lcd_send_cmd(i, LCD_RETURNHOME) != APP_OK)
        {
            app_ret = APP_ERROR;
            continue;
        }
        ...
        if (cfg->has_backlight)
        {
            if (CHARACTER_DISPLAY_BacklightOn(i) != APP_OK)
            {
                app_ret = APP_ERROR;
            }
        }
...
uint8_t CHARACTER_DISPLAY_Clear(CHARACTER_DISPLAY_ID_t unit_id)
{
    ...
    if (lcd_send_cmd(unit_id, LCD_CLEARDISPLAY) != APP_OK)
    {
        return APP_ERROR;
    }
    vTaskDelay(pdMS_TO_TICKS(2)); // Clear display needs ~2ms
...
uint8_t CHARACTER_DISPLAY_SetCursor(CHARACTER_DISPLAY_ID_t unit_id, uint8_t col, uint8_t row)
{
    ...
    if (lcd_send_cmd(unit_id, LCD_SETDDRAMADDR | address) != APP_OK)
    {
        return APP_ERROR;
    }
