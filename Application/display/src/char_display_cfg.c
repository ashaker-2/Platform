...
static uint8_t lcd_send_nibble(CHARACTER_DISPLAY_ID_t unit_id, uint8_t nibble, uint8_t rs_mode)
{
    ...
    // Send with EN high
    if (ECUAL_I2C_Master_Write(cfg->i2c_id, cfg->i2c_address, &data, 1) != E_OK)
    {
        return E_NOK;
    }
...
{
    if (lcd_send_byte(unit_id, cmd, 0) != E_OK)
    { // RS low for command
...
{
    if (lcd_send_byte(unit_id, data, RS_BIT) != E_OK)
    { // RS high for data
...
uint8_t CHARACTER_DISPLAY_Init(void)
{
    ...
    for (uint32_t i = 0; i < CHARACTER_DISPLAY_NUM_CONFIGURATIONS; i++)
    {
        ...
        LOGI(TAG, "Initializing Display %u (I2C: %u, Addr: 0x%02X, %ux%u)",
        ...
        if (lcd_send_nibble(i, 0x30, 0) != E_OK)
        {
            app_ret = E_NOK;
            continue;
        }
        ...
        if (lcd_send_nibble(i, 0x20, 0) != E_OK)
        {
            app_ret = E_NOK;
            continue;
        }                               // Sets 4-bit mode
        ...
        if (cfg->num_rows > 2)
        { // For 4-line displays, still configured as 2-line mode in hardware.
            state->display_function_flags = LCD_4BITMODE | LCD_2LINE | (0x00);
        }

        if (lcd_send_cmd(i, LCD_FUNCTIONSET | state->display_function_flags) != E_OK)
        {
            app_ret = E_NOK;
            continue;
        }
        ...
        if (lcd_send_cmd(i, LCD_DISPLAYCONTROL | state->display_control_flags) != E_OK)
        {
            app_ret = E_NOK;
            continue;
        }
        ...
        // Clear Display
        if (lcd_send_cmd(i, LCD_CLEARDISPLAY) != E_OK)
        {
            app_ret = E_NOK;
            continue;
        }
        ...
        if (lcd_send_cmd(i, LCD_ENTRYMODESET | LCD_ENTRYSHIFTINCREMENT | LCD_ENTRYLEFT) != E_OK)
        {
            app_ret = E_NOK;
            continue;
        }
        ...
        // Return Home
        if (lcd_send_cmd(i, LCD_RETURNHOME) != E_OK)
        {
            app_ret = E_NOK;
            continue;
        }
        ...
        if (cfg->has_backlight)
        {
            if (CHARACTER_DISPLAY_BacklightOn(i) != E_OK)
            {
                app_ret = E_NOK;
            }
        }
        ...
        LOGI(TAG, "Display %u initialization complete.", i);
    }
    return app_ret;
}
...
uint8_t CHARACTER_DISPLAY_Clear(CHARACTER_DISPLAY_ID_t unit_id)
{
    ...
    if (lcd_send_cmd(unit_id, LCD_CLEARDISPLAY) != E_OK)
    {
        return E_NOK;
    }
    vTaskDelay(pdMS_TO_TICKS(2)); // Clear display needs ~2ms
...
uint8_t CHARACTER_DISPLAY_SetCursor(CHARACTER_DISPLAY_ID_t unit_id, uint8_t col, uint8_t row)
{
    ...
    if (col >= cfg->num_columns || row >= cfg->num_rows)
    {
        ...
        if (col >= cfg->num_columns)
            col = cfg->num_columns - 1;
        if (row >= cfg->num_rows)
            row = cfg->num_rows - 1;
    }
    ...
    if (lcd_send_cmd(unit_id, LCD_SETDDRAMADDR | address) != E_OK)
    {
        return E_NOK;
    }
...
uint8_t CHARACTER_DISPLAY_PrintString(CHARACTER_DISPLAY_ID_t unit_id, const char *str)
{
    ...
    for (int i = 0; str[i] != '\0'; i++)
    {
        ...
        { // Handle newline character
            ...
            if (CHARACTER_DISPLAY_SetCursor(unit_id, state->current_col, state->current_row) != E_OK)
            {
                ret = E_NOK;
                break;
            }
        ...
        else
        {
            ...
            if (lcd_send_data(unit_id, (uint8_t)c) != E_OK)
            {
                ret = E_NOK;
                break;
            }
...
uint8_t CHARACTER_DISPLAY_BacklightOn(CHARACTER_DISPLAY_ID_t unit_id)
{
    ...
    if (ECUAL_I2C_Master_Write(cfg->i2c_id, cfg->i2c_address, &data_byte, 1) != E_OK)
    {
        ESP_LOGE(TAG, "Display %u: Failed to turn backlight ON.", unit_id);
        return E_NOK;
    }
    LOGI(TAG, "Display %u backlight ON.", unit_id);
    return E_OK;
}
...
uint8_t CHARACTER_DISPLAY_BacklightOff(CHARACTER_DISPLAY_ID_t unit_id)
{
    ...
    LOGI(TAG, "Display %u backlight OFF.", unit_id);
    return E_OK;
}
