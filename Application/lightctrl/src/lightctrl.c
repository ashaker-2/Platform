    ...
    for (size_t i = 0; i < s_num_light_configurations; i++) {
        ...
        switch (light_cfg->control_type) {
            case LIGHT_CONTROL_TYPE_IO_EXPANDER:
                ...
                if (status == E_OK) 
                {
                    ESP_LOGI(TAG, "Light ID %d (CH423S pin %d) initialized to %s.",
                             light_cfg->light_id, light_cfg->pinNum,
                             (light_cfg->initial_state == LIGHT_STATE_ON) ? "ON" : "OFF");
                } else 
                {
            ...
            case LIGHT_CONTROL_TYPE_GPIO:
                ...
                if (status == E_OK) {
                    ESP_LOGI(TAG, "Light ID %d (GPIO pin %d) initialized to %s.",
                             light_cfg->light_id, light_cfg->pinNum,
                             (light_cfg->initial_state == LIGHT_STATE_ON) ? "ON" : "OFF");
                } else {
...
Status_t LightCtrl_SetState(Light_ID_t light_id, Light_State_t state) {
    ...
    switch (light_cfg->control_type) {
        case LIGHT_CONTROL_TYPE_IO_EXPANDER:
            ...
            if (status == E_OK) {
                ESP_LOGI(TAG, "Light ID %d (CH423S pin %d) set to %s.",
