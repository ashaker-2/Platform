...
static APP_Status_t VentCtrl_ApplyControl(const VentCtrl_Config_t* config, VentCtrl_State_t state, uint8_t speed);
static APP_Status_t VentCtrl_ReadFeedback(const VentCtrl_Config_t* config, uint8_t* actual_speed);
static APP_Status_t VentCtrl_ConvertTachometerToSpeed(uint32_t pulse_count, uint8_t* speed_percent);
...
void VentCtrl_MainFunction(void) {
    ...
    for (uint32_t i = 0; i < VentCtrl_COUNT; i++) {
        ...
        if (VentCtrl_ApplyControl(config, s_commanded_states[i], s_commanded_speeds_percent[i]) != APP_OK) {
        ...
        if (config->feedback_type != VentCtrl_FEEDBACK_TYPE_NONE) {
            if (VentCtrl_ReadFeedback(config, &actual_speed) != APP_OK) {
                RTE_Service_SystemMonitor_ReportFault(FAULT_ID_VENTILATOR_FEEDBACK_FAILURE, SEVERITY_MEDIUM, config->id);
