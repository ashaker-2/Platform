...
static APP_Status_t LightCtrl_ApplyControl(const LightCtrl_Config_t* config, LightCtrl_State_t state, uint8_t brightness);
static APP_Status_t LightCtrl_ReadFeedback(const LightCtrl_Config_t* config, uint8_t* actual_brightness);
static APP_Status_t LightCtrl_ConvertAnalogToBrightness(uint32_t raw_data, const LightCtrl_FeedbackDetails_t* details, uint8_t* brightness);
    ...
    for (uint32_t i = 0; i < LightCtrl_COUNT; i++) {
        ...
        switch (config->type) {
            ...
            default:
                ...
                RTE_Service_SystemMonitor_ReportFault(FAULT_ID_LIGHT_INIT_FAILED, SEVERITY_HIGH, config->id);
                return APP_ERROR;
        }
        ...
        if (config->feedback_type != LightCtrl_FEEDBACK_TYPE_NONE) {
            ...
            if (status != APP_OK) {
                ...
                RTE_Service_SystemMonitor_ReportFault(FAULT_ID_LIGHT_FEEDBACK_FAILURE, SEVERITY_MEDIUM, config->id);
            }
        }
    }
...
void LightCtrl_MainFunction(void) {
    ...
    for (uint32_t i = 0; i < LightCtrl_COUNT; i++) {
        ...
        if (LightCtrl_ApplyControl(config, s_commanded_states[i], s_commanded_brightness_percent[i]) != APP_OK) {
            RTE_Service_SystemMonitor_ReportFault(FAULT_ID_LIGHT_CONTROL_FAILED, SEVERITY_HIGH, config->id);
        ...
        if (config->feedback_type != LightCtrl_FEEDBACK_TYPE_NONE) {
            if (LightCtrl_ReadFeedback(config, &actual_brightness) != APP_OK) {
                RTE_Service_SystemMonitor_ReportFault(FAULT_ID_LIGHT_FEEDBACK_FAILURE, SEVERITY_MEDIUM, config->id);
                ...
                if (s_commanded_states[i] == LightCtrl_STATE_ON && actual_brightness < LightCtrl_MIN_OPERATIONAL_BRIGHTNESS) {
                    RTE_Service_SystemMonitor_ReportFault(FAULT_ID_LIGHT_FEEDBACK_MISMATCH, SEVERITY_MEDIUM, config->id);
...
static APP_Status_t LightCtrl_ReadFeedback(const LightCtrl_Config_t* config, uint8_t* actual_brightness) {
    ...
    switch (config->feedback_type) {
        ...
        case LightCtrl_FEEDBACK_TYPE_LIGHT_SENSOR:
            ...
            break;
        default:
            return APP_ERROR; // Should not be reached if feedback_type != NONE
    }
    return status;
}
